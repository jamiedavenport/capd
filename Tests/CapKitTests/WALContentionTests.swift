...
/// Two real processes on one database file: `CapTestHost` plays the CLI adding captures
...
private func testHostURL() throws -> URL {
    let url = Bundle(for: BundleToken.self).bundleURL.deletingLastPathComponent()
        .appendingPathComponent("CapTestHost", isDirectory: false)
    try #require(
        FileManager.default.isExecutableFile(atPath: url.path),
        "CapTestHost is not built at \(url.path)")
    return url
}

private func withTemporaryPaths(_ body: (StoragePaths) async throws -> Void) async throws {
    let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
        .appendingPathComponent("cap-wal-tests-\(UUID().uuidString)", isDirectory: true)
