...
/// `CAP_WEBKIT_TESTS=1 swift test --filter ReadabilityFetcher`.
@Suite(
    "ReadabilityFetcher",
    .enabled(if: ProcessInfo.processInfo.environment["CAP_WEBKIT_TESTS"] == "1"))
...
/// `CAP_NETWORK_TESTS=1 swift test --filter ReadabilityFetcher`.
@Suite(
    "ReadabilityFetcher live fetch",
    .enabled(if: ProcessInfo.processInfo.environment["CAP_NETWORK_TESTS"] == "1"))
