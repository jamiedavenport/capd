...
struct StoreTaggingTests {
    ...
    @Test("A populated 001 database upgrades in place and stays searchable")
    func upgradeFrom001() throws {
        try withTemporaryPaths { paths in
                ...
                try pool.write { db in
                    try db.execute(
                        sql: """
                            ...
                            VALUES
                                ('link', 'https://example.com/a', 'example.com', 'Legacy row',
            ...
            #expect(try service.search("legacy").map(\.capture.id) == [capture.id])
...
private func withTemporaryPaths(_ body: (StoragePaths) throws -> Void) throws {
    let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
        .appendingPathComponent("cap-tagging-tests-\(UUID().uuidString)", isDirectory: true)
