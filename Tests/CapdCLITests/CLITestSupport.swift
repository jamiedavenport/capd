...
/// The `capd` product lands in the same build directory as this test bundle.
let capdBinary = Bundle(for: BinaryLocator.self).bundleURL
    .deletingLastPathComponent()
    .appendingPathComponent("capd", isDirectory: false)
...
@discardableResult
func capd(_ arguments: [String], stdin: String? = nil, root: URL) throws -> CLIRun {
    let process = Process()
...
func withScratchRoot(_ body: (URL) throws -> Void) throws {
    let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
        .appendingPathComponent("capd-cli-tests-\(UUID().uuidString)", isDirectory: true)
