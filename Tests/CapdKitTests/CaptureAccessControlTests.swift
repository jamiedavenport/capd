...
/// built `CapdKit.swiftmodule` in a separate compiler invocation.
...
private func typecheckAsClient(_ source: String) throws -> TypecheckResult {
    let directory = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
        .appendingPathComponent("capd-typecheck-\(UUID().uuidString)", isDirectory: true)
...
private func clientSearchPaths() throws -> [URL] {
    ...
    let holdingCapdKit = [modules, buildDirectory].first { directory in
        FileManager.default.fileExists(
            atPath: directory.appendingPathComponent("CapdKit.swiftmodule").path)
    }
    let root = try #require(
        holdingCapdKit, "no CapdKit.swiftmodule near the test bundle at \(buildDirectory.path)")
