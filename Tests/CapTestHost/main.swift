...
///
/// Usage: `CapTestHost <store-root> <count> <label>`

let arguments = CommandLine.arguments
guard arguments.count == 4, let count = Int(arguments[2]), count > 0 else {
    FileHandle.standardError.write(Data("usage: CapTestHost <store-root> <count> <label>\n".utf8))
    exit(2)
}
...
do {
    ...
    FileHandle.standardError.write(Data("CapTestHost: \(error)\n".utf8))
    exit(1)
}
