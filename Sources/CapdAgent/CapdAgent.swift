...
@main
struct CapdAgent {
    static let logger = Logger(subsystem: CapdKit.bundleIdentifier, category: "agent")
    ...
    static func main() {
        ...
        case "install" where arguments.count == 1:
            runInstall()
        default:
            FileHandle.standardError.write(Data("usage: capd-agent [install | fetch <url>]\n".utf8))
            exit(EX_USAGE)
        }
    }
    ...
    private static func runInstall() -> Never {
        do {
            ...
            FileHandle.standardError.write(Data("capd-agent: install failed: \(error)\n".utf8))
            exit(EXIT_FAILURE)
        }
    }
