...
/// Reads and sets the share-sheet election for cap's share extension. The toggle in
/// System Settings → General → Login Items & Extensions → Sharing writes the same
/// pluginkit election, so `use` is exactly "enable cap in the share sheet".
enum ShareExtensionElection {
    static let extensionIdentifier = "\(CapKit.bundleIdentifier).share"
