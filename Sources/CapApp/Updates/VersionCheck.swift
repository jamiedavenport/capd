...
extension ReleaseClient {
    static let gitHubReleases = ReleaseClient {
        var request = URLRequest(
            url: URL(string: "https://api.github.com/repos/jamiedavenport/cap/releases/latest")!)
...
final class UpdateChecker {
    static let upgradeCommand = "brew upgrade jamiedavenport/tap/cap"
    ...
    var availableVersion: String? {
        guard let latest = settings.latestKnownVersion,
            ReleaseVersion.isNewer(tag: latest, than: CapKit.version)
        else {
            return nil
        }
        return latest
