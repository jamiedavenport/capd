...
/// Every on-disk location Capd owns, derived from a single root.
///
/// The root is the only thing that has to change to move Capd's data into an app-group
...
public struct StoragePaths: Sendable, Equatable {
    ...
    public static var live: StoragePaths {
        get throws {
            ...
            return StoragePaths(root: support.appendingPathComponent("capd", isDirectory: true))
        }
    }

    public var databaseURL: URL {
        root.appendingPathComponent("capd.sqlite", isDirectory: false)
    }
