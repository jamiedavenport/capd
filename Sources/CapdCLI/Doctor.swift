...
struct Doctor: ParsableCommand {
    ...
    /// The reinstall shells out to `capd-agent install` rather than writing the plist
    ...
    private func repairAgent() -> Bool {
        ...
        guard let agent, FileManager.default.isExecutableFile(atPath: agent.path) else {
            print("Agent: not loaded, and no capd-agent binary found next to capd to reinstall")
            return false
        }
