...
extension OnboardingStep {
    ...
    fileprivate var subtitle: String {
        ...
        case .shareSheet:
            "Any app that shares a link can send it to cap — same pipeline as the "
...
struct OnboardingView: View {
    ...
    private var shareSheet: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            case .enabled:
                Label("cap is in the share sheet", systemImage: "checkmark.circle.fill")
                    .font(.system(size: 12))
                    .foregroundStyle(Theme.success)
                    .transition(.opacity.combined(with: .scale(scale: 0.9, anchor: .leading)))
            case .disabled, .defaulted:
                Button("Add cap to the Share Sheet") {
            ...
            case .unregistered:
                note(
                    "macOS hasn't registered cap's share extension yet. It appears on "
                        + "its own shortly after cap runs from /Applications — nothing "
                        + "to do here.")
            }
        }
