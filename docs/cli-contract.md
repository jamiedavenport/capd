...
**Not stable:** plain (default) output, markdown export, stderr wording, and help text.
These are for people, and change freely.
...
|------|---------|
| 0 | Success. |
| 1 | No results: an empty `search` or `list`, or an id passed to `rm` or `refetch` that does not exist, or `refetch` with nothing to requeue. |
...
| 4 | The enrichment agent is not running: `add --wait` timed out with work still queued. |
...
`--json` is accepted by `add`, `search`, `list`, and `export` as shorthand for JSON
output and wins over `--format`.
