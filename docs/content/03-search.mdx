...
When Apple Intelligence is available, every non-empty search includes an **Ask
Cap** row above the matching captures. Click it to answer the query using the
best matches from your library. You can also start a query with `?` and press
Return:
